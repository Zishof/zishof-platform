...
def rupiah(value: float | int) -> str:
    return "Rp" + f"{round(float(value)):,.0f}".replace(",", ".")
...
SCREEN_SPECS = [
    {
        ...
        "callouts": [
            ((0.08, 0.08, 0.60, 0.14), "Cari/scan produk"),
            ((0.73, 0.66, 0.99, 0.76), "Pilih metode pembayaran"),
            ((0.82, 0.94, 0.99, 0.99), "Bayar dan simpan transaksi"),
        ],
        "narrative": (
    ...
    {
        "file": "04-pr-daftar-100-dokumen.png",
        "title": "Daftar Permintaan Pembelian (PR)",
        "callouts": [
            ((0.11, 0.22, 0.97, 0.37), "Ringkasan 100 dokumen PR"),
            ((0.11, 0.38, 0.98, 0.55), "Sebaran nilai dan status"),
            ((0.54, 0.045, 0.63, 0.09), "Buat PR baru"),
        ],
        "narrative": (
            "Daftar PR menampilkan kebutuhan yang menjadi awal rantai pengadaan. Bukti layar menggunakan 100 dokumen "
            "sampel sehingga peserta dapat mengenali perbedaan status draf, diajukan, disetujui, dan dilanjutkan ke PO. "
            "Periksa nomor dokumen, pemohon, tanggal, jumlah barang, nilai, dan status sebelum membuka rincian. PR baru "
            "harus menjelaskan kebutuhan secara operasional, memiliki barang dan UOM yang benar, serta mengikuti jalur "
            "persetujuan. Karena UAT ini belum membahas COA, jurnal, atau anggaran berbasis akun, pilihan Tanpa anggaran "
            "dapat digunakan bila data anggaran belum disiapkan; keputusan tersebut tidak menghapus jejak persetujuan."
        ),
        "steps": ["Periksa ringkasan jumlah dan nilai PR.", "Gunakan filter untuk menemukan status yang diperlukan.",
                  "Buka satu dokumen dan cocokkan pemohon, barang, UOM, jumlah, serta status.",
                  "Tekan Buat PR untuk latihan baru, simpan, lalu ajukan sesuai hak akses."],
    },
    {
    ...
    {
        "file": "18-omzet-tunai-per-produk-rincian-clickable.png",
        "title": "Rincian omzet Non-Saldo/Tunai per produk",
        "callouts": [
            ((0.30, 0.27, 0.70, 0.73), "Nota penyusun produk terpilih"),
            ((0.31, 0.29, 0.69, 0.38), "Produk dan kelompok pembayaran"),
            ((0.31, 0.60, 0.69, 0.70), "Total rincian untuk rekonsiliasi"),
        ],
        "narrative": (
            "Popup ini terbuka setelah nilai omzet sebuah produk pada laporan Non-Saldo/Tunai diklik. Judul dan "
            "filter di bagian atas menunjukkan produk yang sedang diperiksa serta pembatas kelompok NON_SALDO. "
            "Setiap baris di bawahnya adalah nota yang ikut membentuk omzet produk tersebut, lengkap dengan tanggal, "
            "toko, kasir, metode pembayaran, kuantitas, dan nominal. Jumlah nominal pada bagian bawah harus sama dengan "
            "angka yang diklik pada laporan induk. Dengan cara ini trainer dapat menjelaskan bahwa Tunai/Non-Saldo "
            "bukan sekadar label tampilan, melainkan klasifikasi transaksi yang dapat diaudit hingga nota asal."
        ),
        "steps": ["Klik nilai omzet pada satu produk Non-Saldo/Tunai.", "Pastikan nama produk dan kelompok pembayaran benar.",
                  "Periksa beberapa nomor nota dan metode pembayarannya.", "Cocokkan total popup dengan nilai laporan induk."],
    },
    {
    ...
    {
        "file": "19-omzet-saldo-per-produk-rincian-clickable.png",
        "title": "Rincian omzet Saldo per produk",
        "callouts": [
            ((0.30, 0.27, 0.70, 0.73), "Transaksi saldo penyusun omzet"),
            ((0.31, 0.29, 0.69, 0.38), "Produk dan klasifikasi SALDO"),
            ((0.31, 0.60, 0.69, 0.70), "Nilai rincian yang harus sama"),
        ],
        "narrative": (
            "Rincian Omzet Saldo hanya mengambil transaksi yang metode pembayarannya diklasifikasikan sebagai "
            "pemotong saldo member. Popup mempertahankan konteks produk yang dipilih agar transaksi tunai, transfer, "
            "atau metode non-saldo lain tidak tercampur. Periksa identitas member, nomor nota, tanggal, toko, metode, "
            "dan nominal pada beberapa sampel. Total popup harus sama dengan omzet saldo produk pada tabel induk. "
            "Jika berbeda, pemeriksaan pertama adalah periode, toko, status sinkronisasi transaksi lokal, dan konfigurasi "
            "metode pembayaran; jangan mengubah transaksi hanya untuk membuat angka tampak sama."
        ),
        "steps": ["Klik angka omzet pada laporan Omzet Produk Saldo.", "Pastikan popup memakai klasifikasi SALDO.",
                  "Cocokkan nota dengan Riwayat Penjualan.", "Jumlahkan rincian dan bandingkan dengan laporan induk."],
    },
    {
    ...
    {
        "file": "20-rekap-omzet-per-toko-rincian-clickable.png",
        "title": "Rincian Rekap Omzet per toko",
        "callouts": [
            ((0.30, 0.27, 0.70, 0.73), "Daftar transaksi toko terpilih"),
            ((0.31, 0.29, 0.69, 0.38), "Konteks toko dan kelompok nilai"),
            ((0.31, 0.60, 0.69, 0.70), "Total rincian sumber rekap"),
        ],
        "narrative": (
            "Popup Rekap Omzet memperlihatkan transaksi yang membentuk nilai pada satu toko. Ketika kolom Tunai / "
            "Non-Saldo diklik, rincian hanya memuat kelompok tersebut; ketika Saldo diklik, rincian dibatasi ke metode "
            "pemotong saldo; dan ketika Total Omzet diklik, kedua kelompok digabungkan. Konteks toko dan periode tetap "
            "dibawa dari laporan induk sehingga supervisor tidak perlu mengulang filter. UAT menjalankan ketiga jalur "
            "ini dan membandingkan total popup dengan nilai baris yang diklik. Seluruh selisih harus Rp0 sebelum rekap "
            "digunakan sebagai bahan pelatihan atau keputusan operasional."
        ),
        "steps": ["Klik nilai Non-Saldo, Saldo, atau Total Omzet.", "Periksa konteks toko dan periode.",
                  "Telusuri beberapa nota ke Riwayat Penjualan.", "Pastikan total rincian sama dengan nilai yang diklik."],
    },
    {
...
def annotate(spec: dict) -> Path:
    ...
    # Integration-test Windows dapat menyisakan kanvas hitam di kanan/bawah
    # ketika surface uji lebih besar daripada jendela aplikasi. Area itu bukan
    # bagian UI dan dipangkas sebelum anotasi agar bukti memenuhi halaman.
    rgb = img.convert("RGB")
    mask = ImageChops.difference(rgb, Image.new("RGB", rgb.size, (0, 0, 0))).convert("L")
    mask = mask.point(lambda value: 255 if value > 5 else 0)
    bbox = mask.getbbox()
    if bbox is None:
        raise ValueError(f"Screenshot kosong: {src.name}")
    img = img.crop(bbox)
...
def build_doc():
    ...
    omzet_evidence = json.loads(OMZET_EVIDENCE.read_text(encoding="utf-8"))
    xlsx_evidence = json.loads(XLSX_EVIDENCE.read_text(encoding="utf-8"))
    ...
    add_run(p, "MANUAL PENGGUNA & HASIL UAT END-TO-END", bold=True, size=16, color="2563EB")
    p = doc.add_paragraph()
    ...
    add_table(doc, ["Kelompok pengujian", "Hasil"], [
        ...
        ("Laporan operasional + PDF server", "12/12 lulus"),
        ("Katalog Laporan Omzet", "4/4 lulus"),
        ("API data dan PDF Laporan Omzet", "4/4 lulus"),
        ("Popup rincian clickable Laporan Omzet", "6/6 lulus"),
        ("Ekspor XLSX Laporan Omzet dibuka kembali", "4/4 lulus"),
        ("Rekonsiliasi Laporan Omzet", f"{rupiah(omzet_evidence['reconciliation']['transactionTotal'])}; selisih Rp0"),
    ...
    add_heading(doc, "1.1 Verifikasi Khusus Empat Laporan Omzet", 1)
    add_body(doc,
             "Pembaruan Laporan Omzet untuk POS telah diterapkan pada varian eBisnis, diuji terhadap server live, "
             "dan disertakan pada kandidat rilis ini. Empat laporan berikut menggunakan periode serta toko yang sama. "
             "Setiap nilai atau baris yang mempunyai penanda interaktif dapat diklik untuk membuka nota dan rincian "
             "transaksi penyusunnya. Dengan demikian angka rekap dapat ditelusuri kembali sampai transaksi asal, bukan "
             "hanya dibandingkan sebagai angka ringkasan.")
    omzet_names = {
    ...
    add_table(doc, ["Laporan", "Baris live", "API", "PDF", "XLSX"], [
        (omzet_names[item["id"]], item["rows"], "LULUS", "LULUS", "LULUS")
        for item in omzet_evidence["reports"]
    ], [8, 3, 3, 3, 3])
    add_heading(doc, "Hasil audit khusus", 2)
    add_bullets(doc, [
        "Katalog laporan: 4/4 lulus.",
        "API data dan PDF: 4/4 lulus; setiap PDF memiliki signature %PDF dan isi tidak kosong.",
        "Popup rincian transaksi: 6/6 lulus pada dimensi transaksi, produk, kelompok pembayaran, dan rekap toko.",
        "Ekspor XLSX: 4/4 berhasil dibentuk oleh fungsi aplikasi dan dibuka kembali dengan header serta seluruh baris utuh.",
        f"Rekonsiliasi total: {rupiah(omzet_evidence['reconciliation']['transactionTotal'])} dengan selisih Rp0 terhadap produk dan rekap.",
        "Pengujian aplikasi eBisnis: 851/851 lulus.",
    ])
    add_body(doc,
             "Audit klik memakai enam sampel yang dapat diulang. Lima pemeriksaan memakai periode penuh UAT, sedangkan "
             "pemeriksaan Total Omzet memakai satu hari representatif agar seluruh nota penyusun berada dalam satu "
             "respons. Rekonsiliasi nilai keseluruhan tetap memakai periode 1–10 September 2026 dan menghasilkan "
             "selisih Rp0. Pendekatan ini memisahkan pemeriksaan interaksi popup dari pemeriksaan agregat penuh secara jelas.")
    page_break(doc)

    add_heading(doc, "1.2 Langkah UAT Laporan Omzet pada Perangkat", 1)
    add_numbered(doc, [
        ...
        "Gunakan periode 1–10 September 2026, lalu klik angka atau baris untuk memeriksa transaksi penyusunnya.",
        "Uji tombol Excel dan PDF pada keempat laporan serta buka kembali file hasilnya.",
        "Cocokkan total Transaksi Omzet dengan Non-Saldo + Saldo dan dengan Total Rekap Omzet; selisih harus Rp0.",
    ])
    add_heading(doc, "Kesiapan backend dan paket produksi", 2)
    add_body(doc,
             "Backend yang diperlukan untuk empat Laporan Omzet sudah tersedia pada server live dan audit API "
             "dinyatakan lulus, sehingga skenario ini tidak memerlukan deploy server tambahan. Tindakan di sisi "
             "pengguna adalah memperbarui aplikasi POS ke build eBisnis terbaru. Paket Windows dalam rilis ini "
             "ditujukan untuk UAT internal dan belum memiliki tanda tangan Authenticode. Setelah UAT pengguna "
             "disetujui, paket produksi perlu ditandatangani menggunakan sertifikat resmi organisasi sebelum "
    ...
    add_body(doc,
             ...
             "popup dengan nilai laporan; seluruh enam skenario clickable lulus tanpa selisih.")
    page_break(doc)
    ...
    add_table(doc, ["Skenario", "Baris", "Nilai laporan", "Nilai rincian", "Selisih", "Hasil"], [
        (x["name"], x["rows"], rupiah(x["expectedTotal"]), rupiah(x["actualTotal"]),
         rupiah(x["difference"]), "LULUS" if x["passed"] else "GAGAL")
        for x in omzet_evidence["details"]
    ], [5.5, 2, 3, 3, 2.5, 2])
    add_body(doc,
    ...
    md_lines = [
        ...
        f"| Ekspor XLSX dibuka kembali | {xlsx_evidence['passedCount']}/{xlsx_evidence['totalCount']} lulus |",
        f"| Rekonsiliasi Laporan Omzet | {rupiah(omzet_evidence['reconciliation']['transactionTotal'])}; selisih Rp0 |",
