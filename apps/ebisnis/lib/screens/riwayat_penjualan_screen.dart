...
class _RiwayatPenjualanScreenState extends State<RiwayatPenjualanScreen> {
  ...
  Future<void> _lihatDetail(Map<String, dynamic> row) async {
    try {
      ...
      final subtotalPerBaris = items.map((i) => (i['harga'] as num).toDouble() * (i['qty'] as num).toDouble() - ((i['diskon'] as num?) ?? 0).toDouble()).toList();
              ...
              child: Column(
                ...
                children: [
                  Wrap(spacing: 8, runSpacing: 8, children: [
                    _chipRingkasan('Diskon', _formatRupiah.format(diskonHeader), AppColors.warning),
                    _chipRingkasan('Pajak', _formatRupiah.format(pajakHeader), AppColors.teal),
                    _chipRingkasan('Bayar', _formatRupiah.format(hasil['totalBiaya'] ?? 0), AppColors.success),
                  ]),
                  const Divider(),
  ...
  Future<void> _cetakUlang(Map<String, dynamic> row, Map<String, dynamic> detail, List<Map<String, dynamic>> items) async {
    final itemStruk = items.map((i) => {'nama': i['nama'], 'qty': i['qty'], 'harga': i['harga']}).toList();
    await Navigator.of(context).push(MaterialPageRoute(
      builder: (_) => StrukScreen(
        kode: '${detail['kode'] ?? row['nomorNota'] ?? ''}',
        waktu: _formatWaktu(row['waktu']),
        item: itemStruk,
        total: (detail['totalBiaya'] as num?)?.toDouble() ?? (row['totalBiaya'] as num?)?.toDouble() ?? 0,
        metode: '${row['metode'] ?? ''}',
        ...
        child: ListView(
          ...
          children: [
            Padding(
              ...
              child: Column(
                children: [
                  ...
                  TextField(
                    decoration: const InputDecoration(hintText: 'Cari nama pembeli / nomor nota...', prefixIcon: Icon(Icons.search), border: OutlineInputBorder(), isDense: true),
                    onSubmitted: (v) {
                      _cariPembeli = v;
                      _terapkan();
                    },
            ...
            if (_memuat)
              ...
              const Padding(padding: EdgeInsets.symmetric(vertical: 40), child: Center(child: Text('Belum ada transaksi pada rentang ini.')))
            else ...[
              ..._data.map((row) => Card(
                    margin: const EdgeInsets.symmetric(vertical: 4),
                    child: ListTile(
                      title: Text('${row['nomorNota']}', style: const TextStyle(fontWeight: FontWeight.w600, fontSize: 13)),
                      subtitle: Text('${_formatWaktu(row['waktu'])} · ${row['pembeli']}'),
                      trailing: Column(
                        mainAxisAlignment: MainAxisAlignment.center,
                        crossAxisAlignment: CrossAxisAlignment.end,
                        children: [
                          Text(_formatRupiah.format(row['totalBiaya'] ?? 0), style: const TextStyle(fontWeight: FontWeight.bold)),
                          Text('${row['metode']}', style: const TextStyle(fontSize: 11, color: AppColors.primary)),
                        ],
                      ),
                ...
                Padding(
                  ...
                  child: Row(
                    ...
                    children: [
                      ...
                      Text('Halaman $_halaman / $_totalHalaman ($_total transaksi)'),
