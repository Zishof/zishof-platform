...
/// Pengaturan Alamat Server (padanan `setup.html`/`setup-renderer.js` di
/// desktop-pos-electron) -- diisi sekali di awal (sebelum layar Masuk pernah
/// tampil sama sekali) supaya SATU APK/EXE eBisnis bisa dipakai institusi mana
/// pun, tanpa hardcode domain. Bisa dibuka lagi kapan saja lewat "Ubah Alamat
/// Server" di layar Masuk ([pertamaKali]=false).
///
...
/// "Simpan & Buka Aplikasi" digerbang: host valid DAN tes terakhir sukses
/// UNTUK NILAI FIELD SAAT INI (mengubah field apa pun membatalkan status tes
/// sebelumnya -- tak boleh ada status sukses basi lolos ke server yang beda).
...
class _PengaturanServerScreenState extends State<PengaturanServerScreen> {
  ...
  Future<void> _simpanDanBuka() async {
    ...
    if (widget.pertamaKali) {
      // Jalur normal (_GerbangAwal) yg biasanya memuat ini dilewati sama
      // sekali saat setup pertama kali (layar ini jadi `home` langsung) --
      // muat di sini supaya nama mesin sudah terisi di transaksi pertama.
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      appBar: widget.pertamaKali
          ? null
          : AppBar(title: const Text('Ubah Alamat Server'), backgroundColor: AppColors.sidebarBg, foregroundColor: Colors.white),
      body: SafeArea(
              ...
              Padding(
                ...
                child: Column(
                  ...
                  children: [
                    ...
                    const SizedBox(height: 16),
                    ElevatedButton(
                      onPressed: _bolehSimpan ? _simpanDanBuka : null,
                      ...
                      child: Text(widget.pertamaKali ? 'Simpan & Buka Aplikasi' : 'Simpan'),
                    ),
