...
class _RingkasanTabKepatuhanState extends State<RingkasanTabKepatuhan> {
  ...
  Widget _tabel(String judul, List<Map<String, dynamic>> rows,
      List<(String, String Function(Map<String, dynamic>))> kolom) {
    return Card(
  ...
  Widget build(BuildContext context) {
    ...
    final d = _d!;
    final opnameOverdue =
        ((d['opnameOverdue'] as List?) ?? []).cast<Map<String, dynamic>>();
    final sesiTerbuka =
        ((d['sesiTerbuka'] as List?) ?? []).cast<Map<String, dynamic>>();
    final selisihKas =
        ((d['selisihKas'] as List?) ?? []).cast<Map<String, dynamic>>();
    final diskonManual =
        ((d['diskonManual'] as List?) ?? []).cast<Map<String, dynamic>>();
    final selisihOpname =
        ((d['selisihOpname'] as List?) ?? []).cast<Map<String, dynamic>>();
    final pembatalan = ((d['pembatalanTransaksi'] as List?) ?? [])
      ...
      child: ListView(
        ...
        children: [
          BarisKpi(kartu: [
            KartuKpi(
                label: 'Telat Opname',
                nilai: '${d['jmlTelatOpname'] ?? 0}',
                warna: Colors.red),
            KartuKpi(
                label: 'Sesi Lupa Tutup',
                nilai: '${d['jmlSesiLupaTutup'] ?? 0}',
                warna: Colors.orange),
            KartuKpi(
                label: 'Selisih Kas',
                nilai: formatRupiahDasbor.format(d['totalSelisihKas'] ?? 0),
                warna: const Color(0xFFC0563D)),
            KartuKpi(
                label: 'Diskon Manual',
                nilai: formatRupiahDasbor.format(d['totalDiskonManual'] ?? 0),
                warna: const Color(0xFFB8860B)),
            KartuKpi(
                label: 'Selisih Opname',
                nilai:
                    formatRupiahDasbor.format(d['totalSelisihOpnameRp'] ?? 0),
                warna: Colors.deepOrange),
            KartuKpi(
                label: 'Pembatalan',
                nilai: '${d['jmlPembatalan'] ?? 0}',
                warna: Colors.red),
          ]),
          ...
          _tabel('Sesi Kas Terbuka ≥24 Jam', sesiTerbuka, [
            ...
            (
              'Jam Terbuka',
              (r) => (r['jamTerbuka'] as num?)?.toStringAsFixed(0) ?? '-'
            ),
          ]),
          ...
          _tabel('Diskon Manual Tanpa Aturan (30 hari)', diskonManual, [
            ...
            (
              'Total Diskon',
              (r) => formatRupiahDasbor.format(r['totalDiskon'] ?? 0)
            ),
          ]),
          _tabel('Dampak Selisih Opname (30 hari)', selisihOpname, [
            ...
            (
              'Nilai Rupiah',
              (r) => formatRupiahDasbor.format(r['nilaiRupiah'] ?? 0)
            ),
          ]),
          if (!adaTabelPembatalan)
            Container(
              ...
              child: const Text(
                'Fitur pembatalan transaksi baru ditambahkan -- server perlu di-restart agar tabel ini terisi.',
