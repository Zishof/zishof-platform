...
class _KasKecilScreenState extends State<KasKecilScreen> {
  ...
  Future<void> _muatSemua() async {
    ...
    try {
      ...
      if (!mounted) return;
      setStateIfMounted(() {
        _jenis = ((opsi['jenisKasKecil'] as List?) ?? [])
        ...
        builder: (c, setDialog) {
          Future<void> hitung() async {
            try {
              final res = await ApiClient.instance
                  .aksi('kas_kecil_hitung', {'rincian': rincian});
              setDialog(() {
          ...
          Future<void> muatSaldo() async {
            ...
            try {
              ...
              setDialog(
                  () => saldoKasKecil = (res['saldo'] as num?)?.toDouble());
            } catch (_) {
              setDialog(() => saldoKasKecil = null);
            }
          }
          ...
          return AlertDialog(
            title: Text(ubah
                ? 'Ubah Pengeluaran Kas Kecil'
                : 'Pengeluaran Kas Kecil Baru'),
            content: SizedBox(
              width: 660,
