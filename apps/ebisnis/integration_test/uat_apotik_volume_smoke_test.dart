...
void main() {
  ...
  testWidgets('UAT Apotik volume, transaksi, laporan, dan bukti layar',
    ...
    expect(obat.length, greaterThanOrEqualTo(_minimumKatalog),
        reason:
            'Katalog obat UAT harus berisi sekurangnya $_minimumKatalog baris');
    ringkasan['katalogObatTerbaca'] = obat.length;
    ringkasan['minimumKatalogUntukRun'] = _minimumKatalog;

    final resep = await ApiClient.instance
        .aksi('apotik_resep_list', {'hanya_menunggu': false, 'page_size': 100});
    final daftarResep = _data(resep);
    expect(daftarResep.length, greaterThanOrEqualTo(50),
        reason: 'Resep/racikan UAT harus berisi sekurangnya 50 baris');
    ringkasan['resepRacikanTerbaca'] = daftarResep.length;
