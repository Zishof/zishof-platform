...
const _release = 'apotik-v1.34.23';

void main() {
  IntegrationTestWidgetsFlutterBinding.ensureInitialized();

  testWidgets('UAT E2E Apotik v1.34.23 pada server nyata', (tester) async {
    ...
    if (!uatBatchExists) {
      await _call('apotik_terima_barang', {
        'no_faktur': 'UAT-TERIMA-APT-13423-001',
        ...
        'items': [
          {
            ...
            'keterangan': 'Top-up idempoten berbasis batch UAT v1.34.23',
          }
        ],
      });
    }
    ...
    for (var i = 0; i < _volume; i++) {
      ...
      final finished = await _call('apotik_bayar', {
        'kode': 'UAT-APT-13423-JADI-$number',
      ...
      final compound = await _call('apotik_bayar', {
        'kode': 'UAT-APT-13423-RACIK-$number',
        'resep_id': recipeId,
    ...
    final summary = <String, dynamic>{
      ...
      'katalogObatTerbaca': catalog.length,
      'catatanKatalog': catalog.length >= _volume
          ? 'PASS_VOLUME'
          : 'BLOCKED_DEPLOYMENT_KATALOG_SERVER_HANYA_${catalog.length}_ITEM',
      'resepRacikanTerbaca': recipes.length,
