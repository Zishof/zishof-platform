...
class UpdateChecker {
  ...
  static Future<InfoUpdate?> cekTerbaru({
    ...
    try {
      final resp = await http
          .get(Uri.parse('https://api.github.com/repos/$repoOwner/$repoName/releases/latest'), headers: {'Accept': 'application/vnd.github+json'})
          .timeout(const Duration(seconds: 10));
      if (resp.statusCode != 200) return null;
      final json = jsonDecode(resp.body) as Map<String, dynamic>;
      final tag = (json['tag_name'] as String? ?? '').trim();
      final versiTerbaru = tag.startsWith('v') ? tag.substring(1) : tag;
      if (versiTerbaru.isEmpty || !_lebihBaru(versiTerbaru, versiSaatIni)) return null;
      ...
      for (final a in assets) {
        ...
        if (nama.endsWith('.apk')) urlApk = url;
        if (nama.endsWith('.exe')) urlExe = url;
      }

      return InfoUpdate(
        versi: versiTerbaru,
        catatanRilis: (json['body'] as String? ?? '').trim(),
        urlRilis: (json['html_url'] as String?) ?? 'https://github.com/$repoOwner/$repoName/releases/latest',
        urlApk: urlApk,
        urlExe: urlExe,
      );
    } catch (_) {
  ...
  static List<int> _pecahVersi(String v) {
    ...
    return List.generate(3, (i) => i < bagian.length ? (int.tryParse(bagian[i]) ?? 0) : 0);
  }
}
